...
extractAndLabelImages <- function(label) {
  filenames <- list.files(paste("./data/train/", label, sep = ""), pattern = "*.jpg", full.names = TRUE)
  accum <- c()
  results <- lapply(filenames, function(f) {
    ...
    accum <- as.data.frame(c(accum, imgRgb))
  })
...
splitTrainingAndValidationSets <- function(images) {
  list(images[1:200,], images[201:10000,])
}
```
...
model <- glm(V1 ~ ., family = binomial(), data = training, model = FALSE, y = FALSE)
save(model, file = "shokunin.model")
```
